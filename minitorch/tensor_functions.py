...
class Mul(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        return grad_output.f.mul_zip(grad_output, x2), grad_output.f.mul_zip(
            grad_output, x1
        )
...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor]:
        ...
        sigmoid_grad = grad_output.f.mul_zip(
            sigmoid_a,
            grad_output.f.add_zip(tensor([1]), grad_output.f.neg_map(sigmoid_a)),
        )
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor]:
        ...
        (a,) = ctx.saved_values
        t = a.f.exp_map(a)
        return a.f.mul_zip(t, grad_output)
